...
[format=csv,cols="a"]
|===
실행에 성공한 경우, include::{snippets}/executor/post__execute-code_input-null-data//http-response.adoc[]
|===
...
CAUTION: 결과 식별자가 없는 경우 ``400 Bad Request``를 반환하고, 사용자 정보가 존재하지 않은 경우 ``401 UnAuthentication``을 반환합니다.
